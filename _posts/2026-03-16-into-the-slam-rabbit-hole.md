...
The perception pipeline works. YOLO detects the cones, LiDAR clusters them, the Hungarian algorithm fuses the two streams into a clean list of 3D cone positions. `/cones` gets published. Done. That was the whole of Phase 2.

But then I started actually thinking about what comes after that. The navigation stack needs a *map* of the track to plan a path around it. On the first lap, that map does not exist. The car has to build it while it drives. And to build the map accurately, it needs to know where it is. And to know where it is, it needs a map.

That is the SLAM problem. And once I started reading into it, I genuinely could not stop. This post is the research side — everything I read, what the competitive teams do, and how the different approaches compare. The implementation decisions come next.

---

## The Problem, Stated Plainly

SLAM stands for Simultaneous Localisation and Mapping. The name is accurate: you are doing both things at the same time. There is no pre-existing map to navigate by. GPS is not accurate enough at the sub-metre level needed for a Formula Student track. So the car has to work it out from scratch, in real time, using only what its sensors tell it.

Formally, the full SLAM problem estimates the joint posterior:

```
p(x₁:ₜ, m | z₁:ₜ, u₁:ₜ)
```

Which is just notation for: given all the sensor observations `z` and control inputs `u` up to time `t`, what is the probability distribution over all past poses `x₁:ₜ` and the map `m`? [^13] The map and the full trajectory are estimated simultaneously. In practice, most real-time systems only target the *online* variant, which drops the historical trajectory and only maintains the current pose:

```
p(xₜ, m | z₁:ₜ, u₁:ₜ)
```

The chicken-and-egg structure is real. Accurate mapping requires knowing where you are. Accurate localisation requires a map to localise against. Both must be estimated from noisy data and imperfect odometry. The entire field of SLAM is essentially the question of how to resolve this mutual dependency without one estimate corrupting the other.
...
The track is defined entirely by coloured traffic cones. There are no dynamic obstacles. The environment does not change between laps. And — crucially — every cone is a discrete, identifiable feature. You are not trying to extract structure from an arbitrary scene. The cones *are* the structure.
...
The core idea is to maintain a single joint Gaussian belief over *both* the vehicle pose and all landmark positions in one augmented state vector:

```
μ = [x, y, θ,  l₁ₓ, l₁ᵧ,  l₂ₓ, l₂ᵧ,  ...,  lₙₓ, lₙᵧ]
Σ = (2n+3) × (2n+3) covariance matrix
```

Every landmark you have ever seen lives in that vector alongside your current pose. The covariance matrix `Σ` captures not just individual uncertainties, but the *correlations* between them — if you are uncertain about your own position, that uncertainty propagates into your uncertainty about where the landmarks are. It is a coupled estimate.

The filter runs two alternating steps per time cycle:

**Prediction** — use the motion model to propagate the state forward. Given control inputs (velocity and yaw rate), you can predict roughly where the vehicle went. The motion model Jacobian `G` linearises the non-linear motion around the current estimate, and process noise `R` reflects how uncertain we are about the motion model itself. Landmark positions are left unchanged during prediction — they are fixed in the world frame.

**Update** — for each cone observation, compute the *expected* observation (the range and bearing you would expect to measure to that landmark given the current state estimate), then compare it to the *actual* measurement. The difference — the innovation — updates both the vehicle pose and the matched landmark's position through the Kalman gain `K`. The observation model Jacobian `H` linearises the range-bearing measurement function. The covariance update uses the Joseph form to ensure `Σ` stays positive semi-definite numerically.
...
There is also a known scaling issue. The covariance matrix is `(2n+3) × (2n+3)` where `n` is the number of landmarks, meaning the update step scales as **O(n²)**. On a small Formula Student track with 50–80 cones, this is manageable in real time on capable hardware like the Jetson AGX Orin. On a full endurance course with 200+ cones, the cost starts growing fast.
...
```
F(x) = Σᵢⱼ eᵢⱼ(xᵢ, xⱼ)ᵀ Ωᵢⱼ eᵢⱼ(xᵢ, xⱼ)
```

where `eᵢⱼ` is the error between expected and observed constraints between poses `i` and `j`, and `Ωᵢⱼ` is the information matrix (inverse covariance) of that constraint [^15]. Back-end solvers like g2o, GTSAM, and iSAM2 handle this efficiently using sparse matrix factorisation [^16].
...
The trade-off is implementation complexity. A complete graph-based system requires a robust loop closure detector, a constraint builder, and an integrated sparse optimisation back-end. iSAM2, for example, uses an incremental Bayesian tree structure that is genuinely complex to implement correctly from scratch [^16]. For a general-purpose indoor robot, this is fine — you use a library. For a custom Formula Student cone-based system, you would need to implement the entire front-end yourself anyway, and the back-end integration with a landmark-based representation is non-trivial.
...
The localization half of the SLAM problem — using the cone map to correct the vehicle's pose estimate — is the bit that requires either a full EKF-SLAM filter or a pose-graph back-end to do properly. That is the real challenge.

So I went and built it.

Whether the full EKF-SLAM implementation made it into the deployed system is a different question entirely — and that is exactly what the next post covers.

---

*Citations: refer to the [Literature Review](/fyp-perception-slam/tabs/literature-review/) for full paper details, specifically [6], [7], [8], [13], [14], [15], [16].*
